...
#include "font.h"
#include "system.h"
/* printf() from the LCD ID probe is not used in this bare-metal port. */
...
void LCD_Init(void)
{
	/*==== Clocks: GPIOB,D,E,F,G + FSMC ====*/
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN | RCC_AHB1ENR_GPIODEN |
	                RCC_AHB1ENR_GPIOEEN | RCC_AHB1ENR_GPIOFEN | RCC_AHB1ENR_GPIOGEN;
	RCC->AHB3ENR |= RCC_AHB3ENR_FSMCEN;

	/*==== PB15: general-purpose push-pull output (LCD backlight control) ====*
	 * ALIENTEK 探索者 STM32F407 V1.8+ 的 LCD 背光为高电平点亮 (新版 PCB).
	 * 这里把 PB15 拉高打开背光 (BSRR 低16位是 SET 位).
	 * 旧版 (V1.8-) 才需要低电平点亮. */
	GPIOB->MODER   &= ~(3U << (15 * 2));
	GPIOB->MODER   |=  (1U << (15 * 2));   /* output */
	GPIOB->OTYPER  &= ~(1U << 15);         /* push-pull */
	GPIOB->OSPEEDR |=  (3U << (15 * 2));   /* very high speed */
	GPIOB->PUPDR   &= ~(3U << (15 * 2));   /* no pull */
	GPIOB->BSRR     = (1U << 15);          /* PB15=1 active-high=亮 */

	/*==== FSMC data/control pins -> alternate function AF12 ====
	 * PD: 0,1,4,5,8,9,10,14,15   PE: 7..15   PF12(A6)   PG12(NE4) */
	{
		/* helper: set a pin to AF mode, PP, very-high speed, pull-up */
		static const uint8_t pd_pins[] = {0,1,4,5,8,9,10,14,15};
		static const uint8_t pe_pins[] = {7,8,9,10,11,12,13,14,15};
		for (unsigned k = 0; k < sizeof(pd_pins); k++) {
			uint8_t p = pd_pins[k];
			GPIOD->MODER   &= ~(3U << (p * 2)); GPIOD->MODER   |= (2U << (p * 2)); /* AF */
			GPIOD->OTYPER  &= ~(1U << p);
			GPIOD->OSPEEDR |=  (3U << (p * 2));
			GPIOD->PUPDR   &= ~(3U << (p * 2)); GPIOD->PUPDR |= (1U << (p * 2));
			GPIOD->AFR[p >> 3] &= ~(0xFU << ((p & 7) * 4));
			GPIOD->AFR[p >> 3] |=  (12U  << ((p & 7) * 4));   /* AF12 = FSMC */
		}
		for (unsigned k = 0; k < sizeof(pe_pins); k++) {
			uint8_t p = pe_pins[k];
			GPIOE->MODER   &= ~(3U << (p * 2)); GPIOE->MODER   |= (2U << (p * 2));
			GPIOE->OTYPER  &= ~(1U << p);
			GPIOE->OSPEEDR |=  (3U << (p * 2));
			GPIOE->PUPDR   &= ~(3U << (p * 2)); GPIOE->PUPDR |= (1U << (p * 2));
			GPIOE->AFR[p >> 3] &= ~(0xFU << ((p & 7) * 4));
			GPIOE->AFR[p >> 3] |=  (12U  << ((p & 7) * 4));
		}
		/* PF12 = FSMC_A6 */
		GPIOF->MODER   &= ~(3U << (12 * 2)); GPIOF->MODER   |= (2U << (12 * 2));
		GPIOF->OTYPER  &= ~(1U << 12);
		GPIOF->OSPEEDR |=  (3U << (12 * 2));
		GPIOF->PUPDR   &= ~(3U << (12 * 2)); GPIOF->PUPDR |= (1U << (12 * 2));
		GPIOF->AFR[1]  &= ~(0xFU << ((12 & 7) * 4));
		GPIOF->AFR[1]  |=  (12U  << ((12 & 7) * 4));
		/* PG12 = FSMC_NE4 */
		GPIOG->MODER   &= ~(3U << (12 * 2)); GPIOG->MODER   |= (2U << (12 * 2));
		GPIOG->OTYPER  &= ~(1U << 12);
		GPIOG->OSPEEDR |=  (3U << (12 * 2));
		GPIOG->PUPDR   &= ~(3U << (12 * 2)); GPIOG->PUPDR |= (1U << (12 * 2));
		GPIOG->AFR[1]  &= ~(0xFU << ((12 & 7) * 4));
		GPIOG->AFR[1]  |=  (12U  << ((12 & 7) * 4));
	}

	/*==== FSMC NOR/SRAM Bank1, sub-bank 4 (NE4) — 16-bit, extended mode ====
	 * Registers: BTCR[6]=BCR4, BTCR[7]=BTR4 (read timing), BWTR[6]=BWTR4 (write timing).
	 * Timing values mirror the ALIENTEK reference driver at 168MHz HCLK. */
	FSMC_Bank1->BTCR[6] = FSMC_BCR_MTYP_SRAM | FSMC_BCR_MWID_16 |
	                      FSMC_BCR_WREN | FSMC_BCR_EXTMOD;
	/* Read timing (BTR4): ADDSET=0xF, DATAST=60, mode A (bits[29:28]=00).
	 * 注: 168MHz 下 HCLK=6ns, DATAST=60 = 360ns. 部分 LCD 控制器 (尤其是
	 * NT35310/SSD1963) 在初次读 ID 时响应较慢, 这里把读时序拉到更长以
	 * 保证 ID 正确识别; 识别完成后再调快. */
	FSMC_Bank1->BTCR[7] = (0xFU << 0) | (0xFFU << 8);
	/* Write timing (BWTR4): ADDSET=9, DATAST=60, mode A */
	FSMC_Bank1E->BWTR[3] = (9U << 0) | (60U << 8);
	/* Enable the bank */
	FSMC_Bank1->BTCR[6] |= FSMC_BCR_MBKEN;
	...
	/* ID 读取失败时的兜底: 强制按 ILI9341 初始化 (ALIENTEK 探索者标配) */
	if (lcddev.id != 0x9341 && lcddev.id != 0x5310 && lcddev.id != 0x5510 &&
	    lcddev.id != 0x1963 && lcddev.id != 0x6804 && lcddev.id != 0xC505) {
	    lcddev.id = 0x9341;
	}
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//如果是这几个IC,则设置WR时序为最快
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FSMC_Bank1E->BWTR[3]&=~(0XF<<0);//地址建立时间(ADDSET)清零
		FSMC_Bank1E->BWTR[3]&=~(0XF<<8);//数据保存时间清零
		FSMC_Bank1E->BWTR[3]|=5<<0;		//ADDSET=5 => 30ns  (HCLK=6ns)
		FSMC_Bank1E->BWTR[3]|=15<<8; 	//DATAST=15 => 90ns  (留余量给 ILI9341 t_WR)
	}else if(lcddev.id==0X6804||lcddev.id==0XC505)	//6804/C505速度上不去,得降低
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FSMC_Bank1E->BWTR[3]&=~(0XF<<0);//地址建立时间(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[3]&=~(0XF<<8);//数据保存时间清零
		FSMC_Bank1E->BWTR[3]|=10<<0;	//地址建立时间(ADDSET)为10个HCLK =60ns  	 
		FSMC_Bank1E->BWTR[3]|=12<<8; 	//数据保存时间(DATAST)为6ns*13个HCLK=78ns
	}
 	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID   
	...
	LCD_LED_SET(1);			//点亮背光 (PB15, 新版 ALIENTEK 探索者 active-high)
	LCD_Clear(WHITE);
}  
//清屏函数
//color:要清屏的填充色
